...
const getSearch = zpid => {
  let property = {}
  return dispatch => (
    fetch(`https://cors-anywhere.herokuapp.com/zillow.com/webservice/GetDeepComps.htm?zws-id=${ZWS_ID}&zpid=${zpid}&count=5`)
...
export const getZPID = search => {
  return dispatch => {
    fetch(`https://cors-anywhere.herokuapp.com/zillow.com/webservice/GetSearchResults.htm?zws-id=${ZWS_ID}&address=${encodeURIComponent(search.address)}&citystatezip=${encodeURIComponent(search.citystate)}`)
      .then(response => response.text())
      .then(text => (new window.DOMParser()).parseFromString(text, "text/xml"))
